...
$approved_posts = post::where('status', $status)->get(['id', 'title','category_id', 'user_id', 'image', 'content',  'created_at','status']);
...
//counting posts comment in comment table
...
$post_tags = post::find($postId)->tags;
...
//get post username from users table
public function post_author($id){
$author = post::find($id);
return $author->user->username;
}

//get post user_id from users table
